...
| `agent` | Only the agent identity that wrote it | Working notes private to one agent |
...
<Note>
  `scope: "agent"` resolves against the `X-Vex-Agent` header. If your client
  does not send one, every agent shares a single default identity — and
  `agent` scope stops meaning very much. Set the header.
</Note>
...
`X-Vex-Project` header, carrying a git remote URL or a project slug.
